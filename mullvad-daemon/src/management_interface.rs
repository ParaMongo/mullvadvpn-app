    ...
    async fn connect_tunnel(&self, _: Request<()>) -> ServiceResult<()> {
        ...
        self.send_command_to_daemon(DaemonCommand::SetTargetState(tx, TargetState::Secured))
            ...
            .and_then(|result| match result {
                Ok(()) => Ok(Response::new(())),
                Err(()) => Err(Status::new(
                    mullvad_management_interface::Code::from(-900),
                    ...
                    RestError::ApiError(StatusCode::BAD_REQUEST, message) => {
                        match &message.as_str() {
                            &mullvad_rpc::INVALID_VOUCHER => {
                                Status::new(mullvad_management_interface::Code::NotFound, message)
                            }

                            &mullvad_rpc::VOUCHER_USED => Status::new(
                                mullvad_management_interface::Code::ResourceExhausted,
                                message,
                            ),
...
fn map_rest_account_error(error: RestError) -> Status {
    match error {
        ...
        {
            Status::new(mullvad_management_interface::Code::Unauthenticated, message)
        }
        _ => Status::internal("internal error"),
    }
}
